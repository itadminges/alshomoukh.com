  ...
  return (
    <footer id="site-footer" className="scroll-mt-20 bg-[#0B3958] text-white pb-0 pt-0 overflow-hidden border-t border-white/5 md:pb-7 md:pt-12">
      <div className="mx-auto max-w-[1440px] px-0 md:px-8 lg:px-12">
        <div className="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-12 md:gap-8 lg:gap-6 xl:gap-8 mb-0 md:mb-8">

          {/* Brand Column */}
          <div className="lg:col-span-3 flex flex-col items-center md:items-start gap-5 md:gap-4 order-last lg:order-first mt-10 pt-0 px-6 md:px-0 md:pt-0 md:border-t-0 md:mt-0 text-center md:text-left">
            ...
            <div className="mt-1 flex gap-2.5 md:mt-0.5">
              {socialLinks.map((social) => (
                <Link
                  ...
                  className="w-11 h-11 md:w-8.5 md:h-8.5 rounded-full border border-white/15 flex items-center justify-center text-white/70 hover:text-white hover:border-white/30 hover:bg-white/5 transition-all duration-500 shrink-0"
                  aria-label={social.label}
                >
                  <social.icon className="w-4 h-4 md:w-3.5 md:h-3.5 shrink-0" />
                </Link>
              ))}
            </div>
          </div>
          ...
          {footerNav.map((cluster) => (
            <div key={cluster.title} className="lg:col-span-2 flex flex-col items-center md:items-start w-full border-b border-white/10 md:border-b-0 md:py-0">
              ...
              >
                <h2 className="text-[13px] md:text-[11px] font-extrabold tracking-[0.16em] md:tracking-[0.22em] uppercase text-white md:text-white/40 text-center md:text-left md:mb-4">
                  {cluster.title}
                </h2>
              ...
              >
                <ul className="flex flex-col gap-5 md:gap-2.5 items-center md:items-start text-center md:text-left pb-2 md:pb-0">
                  {cluster.links.map((link) => (
                    <li key={link.label}>
                      <Link
                        href={link.href}
                        className="text-[15px] md:text-[12.5px] font-medium md:font-semibold text-white/60 hover:text-white transition-all duration-300 tracking-wide md:uppercase group flex items-center"
                      >
                        <span className="relative">
                          {link.label}
                          <span className="absolute -bottom-0.5 left-0 w-0 h-[1.5px] bg-secondary/50 transition-all duration-300 group-hover:w-full" />
                        </span>
                      </Link>
                    </li>
                  ))}
          ...
          {/* Contact Column */}
          <div className="lg:col-span-3 flex flex-col items-center md:items-start w-full border-b border-white/10 md:border-b-0 md:py-0">
            <button
              type="button"
            ...
            >
              <ul className="flex flex-col gap-6 md:gap-3.5 w-full items-center md:items-start text-center md:text-left pb-2 md:pb-0">
                <li className="flex flex-col gap-2 md:gap-1 group cursor-default items-center md:items-start">
                  <div className="flex items-center gap-2 text-white/40 group-hover:text-secondary transition-colors duration-300">
                    <span className="inline-block md:hidden shrink-0"><MapPin className="h-4 w-4 shrink-0" /></span>
                    <span className="hidden md:inline-block shrink-0"><MapPin className="h-3 w-3 shrink-0" /></span>
                    <span className="text-[12px] md:text-[10px] font-bold tracking-widest uppercase">Location</span>
                  </div>
                  <span className="text-[15px] md:text-[12px] font-medium text-white/70 leading-relaxed md:pl-4 md:border-l border-white/10 group-hover:border-secondary/30 transition-colors duration-300">
                ...
                <li className="flex flex-col gap-2 md:gap-1 group cursor-default items-center md:items-start">
                  <div className="flex items-center gap-2 text-white/40 group-hover:text-secondary transition-colors duration-300">
                    ...
                    <span className="hidden md:inline-block shrink-0"><Phone className="h-3 w-3 shrink-0" /></span>
                    <span className="text-[12px] md:text-[10px] font-bold tracking-widest uppercase">Inquiries</span>
                  </div>
                  <a
                    href="tel:+96824284771"
                    className="whitespace-nowrap text-[15px] md:text-[12px] font-medium text-white/70 hover:text-white md:pl-4 md:border-l border-white/10 group-hover:border-secondary/30 transition-colors duration-300"
                  >
                    +968 24284771
                  </a>
                  <a
                ...
                <li className="flex flex-col gap-2 md:gap-1 group cursor-default items-center md:items-start">
                  <div className="flex items-center gap-2 text-white/40 group-hover:text-secondary transition-colors duration-300">
                    ...
                    <span className="hidden md:inline-block shrink-0"><Mail className="h-3 w-3 shrink-0" /></span>
        ...
        {/* Lower Footer */}
        <div className="mt-10 border-t border-white/10 flex flex-col md:mt-0 md:pt-5 md:flex-row md:justify-between md:items-center md:gap-10">
          <div className="flex flex-col md:flex-row items-center gap-3 md:gap-6 order-2 md:order-1 px-6 py-7 md:px-0 md:py-0">
            <p className="max-w-[260px] text-center text-[12px] font-normal leading-relaxed text-white/55 tracking-normal md:max-w-none md:text-left md:text-[10.5px] md:font-bold md:text-white/30 md:tracking-[0.2em] md:uppercase">
              Copyright &copy; {new Date().getFullYear()}
            </p>
            <div className="hidden md:block h-3 w-[1px] bg-white/10" />
            <div className="flex items-center gap-2 text-white/45">
              <Link href="/privacy" className="inline-flex min-h-11 items-center text-[12px] font-normal tracking-normal hover:text-white transition-colors md:min-h-0 md:text-[10.5px] md:font-bold md:text-white/30 md:tracking-[0.2em] md:uppercase">Privacy Policy</Link>
              <span className="text-[10px] md:hidden" aria-hidden="true">•</span>
              <Link href="/terms" className="inline-flex min-h-11 items-center text-[12px] font-normal tracking-normal hover:text-white transition-colors md:min-h-0 md:text-[10.5px] md:font-bold md:text-white/30 md:tracking-[0.2em] md:uppercase">Terms of Service</Link>
            </div>
          </div>
          ...
          <div className="flex w-full items-center justify-center gap-2.5 border-b border-white/10 px-6 py-8 order-1 md:order-2 md:w-auto md:justify-start md:border-b-0 md:px-0 md:py-0">
            ...
            <Image
              ...
              width={200}
              height={51}
              className="h-7 w-auto object-contain opacity-100 transition-opacity duration-300 md:h-7 shrink-0"
            />
          </div>
        </div>
      </div>
